...
std::string Player::getDescription(int32_t lookDistance) const
{
	...
	if(marriage && IOLoginData::getInstance()->getNameByGuid(marriage, tmp))
	{
		...
		s << (sex % 2 ? "Marido" : "Esposa") << " do(a) " << tmp;
	}
...
void Player::addSkillAdvance(skills_t skill, uint32_t count, bool useMultiplier/* = true*/)
{
	...
	while(skills[skill][SKILL_TRIES] + count >= nextReqTries)
	{
		...
		s.str("");
		s << "Você evoluiu o " << getSkillName(skill);
...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		case RET_DESTINATIONOUTOFREACH:
			sendCancel("O destino está fora do alcance.");
			break;

		case RET_NOTMOVEABLE:
			sendCancel("Você não pode mover esse objeto.");
			break;

		case RET_DROPTWOHANDEDITEM:
			sendCancel("Largue o objeto com duas mãos primeiro.");
			break;

		case RET_BOTHHANDSNEEDTOBEFREE:
			sendCancel("As duas mãos precisam estar livres.");
			break;

		case RET_CANNOTBEDRESSED:
			sendCancel("Você não pode colocar este objeto lá.");
			break;

		case RET_PUTTHISOBJECTINYOURHAND:
			sendCancel("Coloque este objeto em sua mão.");
			break;

		case RET_PUTTHISOBJECTINBOTHHANDS:
			sendCancel("Coloque este objeto em ambas as mãos.");
			break;
		...
		case RET_FIRSTGODOWNSTAIRS:
			sendCancel("Primeiro desça as escadas.");
			break;

		case RET_FIRSTGOUPSTAIRS:
			sendCancel("Primeiro vá lá em cima.");
			break;

		case RET_NOTENOUGHCAPACITY:
			sendCancel("Este objeto é muito pesado.");
			break;

		case RET_CONTAINERNOTENOUGHROOM:
			sendCancel("Você não pode colocar mais objetos neste recipiente.");
			break;
		...
		case RET_CANNOTPICKUP:
			sendCancel("Você não pode pegar este objeto.");
			break;
		...
		case RET_THISISIMPOSSIBLE:
			sendCancel("Isso é impossível.");
			break;

		case RET_PLAYERISPZLOCKED:
			sendCancel("Você não pode entrar em uma zona de proteção depois de atacar outro jogador.");
			break;

		case RET_PLAYERISNOTINVITED:
			sendCancel("Você não está convidado.");
			break;

		case RET_CREATUREDOESNOTEXIST:
			sendCancel("A criatura não existe.");
			break;

		case RET_DEPOTISFULL:
			sendCancel("Você não pode colocar mais itens neste depósito.");
			break;
		...
		case RET_PLAYERWITHTHISNAMEISNOTONLINE:
			sendCancel("O jogador com este nome não está online.");
			break;

		case RET_NOTREQUIREDLEVELTOUSERUNE:
			sendCancel("Você não possui ninjutsu necessário para usar esta runa.");
			break;

		case RET_YOUAREALREADYTRADING:
			sendCancel("Você já está negociando.");
			break;

		case RET_THISPLAYERISALREADYTRADING:
			sendCancel("Este jogador já está negociando.");
			break;

		case RET_YOUMAYNOTLOGOUTDURINGAFIGHT:
			sendCancel("Você pode não sair durante ou imediatamente após uma luta!");
			break;

		case RET_DIRECTPLAYERSHOOT:
			sendCancel("Você não tem permissão para atirar diretamente nos jogadores.");
			break;

		case RET_NOTENOUGHLEVEL:
			sendCancel("Você não tem nível suficiente.");
			break;

		case RET_NOTENOUGHMAGICLEVEL:
			sendCancel("Você não tem ninjutsu suficiente.");
			break;
		...
		case RET_NOTENOUGHSOUL:
			sendCancel("Você não tem alma suficiente.");
			break;
		...
		case RET_CANONLYUSETHISRUNEONCREATURES:
			sendCancel("Você só pode usar essa runa nas criaturas.");
			break;
		...
		case RET_CREATUREISNOTREACHABLE:
			sendCancel("A criatura não é acessível.");
			break;

		case RET_ACTIONNOTPERMITTEDINPROTECTIONZONE:
			sendCancel("Esta ação não é permitida em uma zona de proteção.");
			break;
		...
		case RET_YOUMAYNOTATTACKTHISCREATURE:
			sendCancel("Você não pode atacar essa criatura.");
			break;

		case RET_YOUMAYNOTATTACKAPERSONINPROTECTIONZONE:
			sendCancel("Você não pode atacar uma pessoa em uma zona de proteção.");
			break;

		case RET_YOUMAYNOTATTACKAPERSONWHILEINPROTECTIONZONE:
			sendCancel("Você não pode atacar uma pessoa enquanto estiver em uma zona de proteção.");
			break;

		case RET_YOUCANONLYUSEITONCREATURES:
			sendCancel("Você só pode usá-lo em criaturas.");
			break;

		case RET_TURNSECUREMODETOATTACKUNMARKEDPLAYERS:
			sendCancel("Desligue o modo seguro se quiser realmente atacar jogadores não marcados.");
			break;
		...
		case RET_YOURVOCATIONCANNOTUSETHISSPELL:
			sendCancel("Sua vocação não pode usar esse jutsu.");
			break;

		case RET_YOUNEEDAWEAPONTOUSETHISSPELL:
			sendCancel("Você precisa equipar uma arma para usar esse jutsu.");
			break;

		case RET_PLAYERISPZLOCKEDLEAVEPVPZONE:
			sendCancel("Você não pode deixar uma zona pvp depois de atacar outro jogador.");
			break;
		...
		case RET_ACTIONNOTPERMITTEDINANOPVPZONE:
			sendCancel("Esta ação não está permitida em uma zona non-pvp.");
			break;

		case RET_YOUCANNOTLOGOUTHERE:
			sendCancel("Você não pode deslogar aqui.");
			break;

		case RET_YOUNEEDAMAGICITEMTOCASTSPELL:
			sendCancel("Você precisa de um item especial para lançar este jutsu.");
			break;

		case RET_CANNOTCONJUREITEMHERE:
			sendCancel("Você não pode conjurar itens aqui.");
			break;

		case RET_YOUNEEDTOSPLITYOURSPEARS:
			sendCancel("Você precisa dividir suas lanças primeiro.");
			break;

		case RET_NAMEISTOOAMBIGUOUS:
			sendCancel("O nome é muito ambíguo.");
			break;
		...
		case RET_YOUMAYNOTCASTAREAONBLACKSKULL:
			sendCancel("Você não pode lançar jutsus de área enquanto você está black skull.");
			break;

		case RET_TILEISFULL:
			sendCancel("Você não pode adicionar mais itens neste tile.");
			break;
...
void Player::onCreatureAppear(const Creature* creature)
{
	...
	if(g_config.getBool(ConfigManager::DISPLAY_LOGGING))
		std::cout << name << " Se conectou." << std::endl;
}
...
#else
		std::cout << "Jogador " << getName() << " Não pôde ser salvo." << std::endl;
#endif
}
...
void Player::removeMessageBuffer()
{
	...
	if(!hasFlag(PlayerFlag_CannotBeMuted) && maxBuffer != 0 && messageBuffer <= maxBuffer + 1)
	{
		if(++messageBuffer > maxBuffer)
		{
			...
			char buffer[50];
			sprintf(buffer, "Você foi mutado durante %d segundos.", muteTime);
...
void Player::drainHealth(Creature* attacker, CombatType_t combatType, int32_t damage)
{
	...
	char buffer[150];
	if(attacker)
		sprintf(buffer, "Você perdeu %d de Vida Devido a um ataque de %s.", damage, attacker->getNameDescription().c_str());
	else
		sprintf(buffer, "Você perdeu %d de Vida.", damage);
...
void Player::drainMana(Creature* attacker, CombatType_t combatType, int32_t damage)
{
	...
	char buffer[150];
	if(attacker)
		sprintf(buffer, "Você perdeu %d Chakra Bloqueando um ataque por %s.", damage, attacker->getNameDescription().c_str());
	else
		sprintf(buffer, "Você perdeu %d Chakra.", damage);
...
void Player::addManaSpent(uint64_t amount, bool useMultiplier/* = true*/)
{
	...
	while(manaSpent + amount >= nextReqMana)
	{
		...
		char advMsg[50];
		sprintf(advMsg, "Você avançou o nível de Ninjutsu %d.", magLevel);
...
void Player::addExperience(uint64_t exp)
{
	...
	if(prevLevel != level)
	{
		...
		char advMsg[60];
		sprintf(advMsg, "Você evoluiu do nivel %d para o nivel %d.", prevLevel, level);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	if(prevLevel != level)
	{
		...
		char advMsg[90];
		sprintf(advMsg, "Se fodeu caiu do Nivel %d para o Nivel %d.", prevLevel, level);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
	}
...
Item* Player::createCorpse(DeathList deathList)
{
	...
	std::stringstream ss;
	ss << "Você reconhece que " << getNameDescription() << ". " << (sex % 2 ? "Ele" : "Ela") << " foi morto(a) por ";
	if(deathList[0].isCreatureKill())
	{
		...
		if(deathList[0].getKillerCreature()->getMaster())
			ss << " invocado por " << deathList[0].getKillerCreature()->getMaster()->getNameDescription();
	}
	else
	...
	if(deathList.size() > 1)
	{
		if(deathList[0].getKillerType() != deathList[1].getKillerType())
		{
			if(deathList[1].isCreatureKill())
			{
				...
				if(deathList[1].getKillerCreature()->getMaster())
					ss << " invocado por " << deathList[1].getKillerCreature()->getMaster()->getNameDescription();
			}
			else
		...
		else if(deathList[1].isCreatureKill())
		{
			if(deathList[0].getKillerCreature()->getName() != deathList[1].getKillerCreature()->getName())
			{
				ss << " e por " << deathList[1].getKillerCreature()->getNameDescription();
				if(deathList[1].getKillerCreature()->getMaster())
					ss << " invacado por " << deathList[1].getKillerCreature()->getMaster()->getNameDescription();
			}
		}
...
bool Player::addVIP(uint32_t _guid, std::string& name, bool isOnline, bool internal/* = false*/)
{
	...
	{
		if(!internal)
			sendTextMessage(MSG_STATUS_SMALL, "Você não pode se adicionar.");
	...
	{
		if(!internal)
			sendTextMessage(MSG_STATUS_SMALL, "Você não pode adicionar mais amigos.");
...
void Player::onAddCombatCondition(ConditionType_t type, bool hadCondition)
{
	...
	switch(type)
	{
		...
		case CONDITION_MANASHIELD:
			tmp = "Protegido por um escudo mágico";
			break;
	...
	if(!tmp.empty())
		sendTextMessage(MSG_STATUS_DEFAULT, "Você é " + tmp + ".");
}
...
void Player::onAttackedCreatureDrain(Creature* target, int32_t points)
{
	...
	char buffer[100];
	sprintf(buffer, "Você causou %d Dano a(o) %s.", points, target->getNameDescription().c_str());
...
void Player::onSummonAttackedCreatureDrain(Creature* summon, Creature* target, int32_t points)
{
	...
	char buffer[100];
	sprintf(buffer, "Você %s Causou %d Dano a(o) %s.", summon->getName().c_str(), points, target->getNameDescription().c_str());
	sendTextMessage(MSG_EVENT_DEFAULT, buffer);
}
...
bool Player::addUnjustifiedKill(const Player* attacked)
{
	...
	{
		char buffer[90];
		sprintf(buffer, "Atenção! O assassinato de %s Não foi justificado.",
	...
	if(!g_config.getBool(ConfigManager::USE_BLACK_SKULL))
	{
		...
		if(!IOBan::getInstance()->addAccountBanishment(accountId, (now + g_config.getNumber(
			ConfigManager::KILLS_BAN_LENGTH)), 20, ACTION_BANISHMENT, "Matar jogador injustificado.", 0, guid))
			return true;

		sendTextMessage(MSG_INFO_DESCR, "Você foi banido.");
...
void Player::manageAccount(const std::string &text)
{
	...
	switch(accountManager)
	{
		case MANAGER_NAMELOCK:
		{
			if(!talkState[1])
			{
				...
				if(managerString.length() < 4)
					msg << "O nome que deseja é muito curto, selecione um nome mais longo.";
				else if(managerString.length() > 20)
					msg << "O nome que você deseja é muito longo, selecione um nome mais curto.";
				else if(!isValidName(managerString))
					msg << "Esse nome parece conter símbolos inválidos, escolha outro nome.";
				else if(IOLoginData::getInstance()->playerExists(managerString, true))
					msg << "Um jogador com esse nome já existe, escolha outro nome.";
				else
				{
					...
					if(tmp.substr(0, 4) != "god " && tmp.substr(0, 3) != "cm " && tmp.substr(0, 3) != "gm ")
					{
						talkState[1] = true;
						talkState[2] = true;
						msg << managerString << ", Você tem certeza?";
					}
					else
						msg << "Seu personagem não é um membro da equipe, por favor me fale outro nome!";
				}
			}
			else if(checkText(text, "no") && talkState[2])
			{
				talkState[1] = talkState[2] = false;
				msg << "O que mais você gostaria de nomear seu personagem?";
			}
			else if(checkText(text, "yes") && talkState[2])
			{
				if(!IOLoginData::getInstance()->playerExists(managerString, true))
				{
						...
						IOBan::getInstance()->removePlayerBanishment(tmp, PLAYERBAN_LOCK))
					{
						...
						talkState[1] = true;
						talkState[2] = false;
						msg << "O seu personagem foi renomeado com sucesso, agora você poderá fazer o login sem problemas.";
					}
					else
					{
						talkState[1] = talkState[2] = false;
						msg << "Falha ao alterar o seu nome, tente novamente.";
					}
				}
				else
				{
				...
				msg << "Desculpe, mas não consigo entender você, tente repetir isso!";
		...
		case MANAGER_ACCOUNT:
		{
			...
			if(checkText(text, "cancel") || (checkText(text, "account") && !talkState[1]))
			{
				...
				msg << "Você quer mudar seu 'password', request a 'recovery key', adicionar um 'character', ou 'delete' um character?";
			}
			else if(checkText(text, "delete") && talkState[1])
			{
				talkState[1] = false;
				talkState[2] = true;
				msg << "Qual personagem você gostaria de excluir?";
			}
			else if(talkState[2])
			{
				std::string tmp = text;
				...
				if(!isValidName(tmp, false))
					msg << "Esse nome contém caracteres inválidos, tente dizer seu nome novamente, você pode ter digitado isso de forma errada.";
				else
				{
					talkState[2] = false;
					talkState[3] = true;
					managerString = tmp;
					msg << "Você realmente deseja excluir o personagem chamado " << managerString << "?";
				}
			}
			else if(checkText(text, "yes") && talkState[3])
			{
				switch(IOLoginData::getInstance()->deleteCharacter(managerNumber, managerString))
				{
					case DELETE_INTERNAL:
						msg << "Ocorreu um erro ao excluir o seu personagem. Ou o personagem não pertence a você ou não existe.";
						break;

					case DELETE_SUCCESS:
						msg << "Seu personagem foi excluído.";
						break;

					case DELETE_HOUSE:
						msg << "Seu personagem é dono de uma casa. Para garantir que você realmente deseja perder sua casa, excluindo seu personagem, você deve entrar e sair da casa ou passar para outra pessoa primeiro.";
						break;

					case DELETE_LEADER:
						msg << "Seu personagem é o líder de uma guilda. Você precisa dissolver ou passar a liderança de outra pessoa para excluir seu personagem.";
						break;

					case DELETE_ONLINE:
						msg << "O personagem com esse nome está atualmente online, para excluir o personagem ele deve estar offline.";
						break;
				}
			...
			else if(checkText(text, "no") && talkState[3])
			{
				talkState[1] = true;
				talkState[3] = false;
				msg << "Diga-me qual o personagem que deseja excluir.";
			}
			else if(checkText(text, "password") && talkState[1])
			{
				talkState[1] = false;
				talkState[4] = true;
				msg << "Diga-me sua nova senha, por favor.";
			}
			else if(talkState[4])
			{
				std::string tmp = text;
				trimString(tmp);
				if(tmp.length() < 6)
					msg << "Essa senha é muito curta, pelo menos 6 dígitos são necessários. Selecione uma senha mais longa.";
				else if(!isValidPassword(tmp))
					msg << "Sua senha contém caracteres inválidos ... por favor, me fale outra.";
				else
				{
					talkState[4] = false;
					talkState[5] = true;
					managerString = tmp;
					msg << "Deseja que '" << managerString << "' essa seja sua nova senha?";
				}
			}
			else if(checkText(text, "yes") && talkState[5])
			{
				...
				msg << "Sua seja foi alterada com sucesso.";
			}
			...
			else if(checkText(text, "character") && talkState[1])
			{
				if(account.charList.size() <= 15)
				{
					talkState[1] = false;
					talkState[6] = true;
					msg << "Como você gostaria que fosse o nome do seu personagem?";
				}
				else
				{
					talkState[1] = true;
					...
					msg << "Sua conta atingiu o limite de 15 jogadores, você pode 'delete' Um personagem se quiser criar um novo.";
				}
			}
			else if(talkState[6])
			{
				...
				else
				{
					...
					if(tmp.substr(0, 4) != "god " && tmp.substr(0, 3) != "cm " && tmp.substr(0, 3) != "gm ")
					{
						...
						msg << "Seu personagem não é um membro da equipe, por favor me avise outro nome!";
				}
			}
			else if(checkText(text, "no") && talkState[7])
			{
				talkState[6] = true;
				talkState[7] = false;
				msg << "Você gostaria de nomear seu personagem?";
			}
			else if(checkText(text, "yes") && talkState[7])
			{
				talkState[7] = false;
				talkState[8] = true;
				msg << "Se o seu personagem for um 'male' ou 'female'.";
			}
			else if(talkState[8] && (checkText(text, "female") || checkText(text, "male")))
			{
				...
				{
					msg << "Uma mulher, você tem certeza?";
					managerSex = PLAYERSEX_FEMALE;
				}
				else
				{
					msg << "Um homem, você tem certeza?";
			...
			else if(checkText(text, "no") && talkState[9])
			{
				talkState[8] = true;
				talkState[9] = false;
				msg << "Diga-me ... você gostaria de ser um 'male' ou 'female'?";
			}
			else if(checkText(text, "yes") && talkState[9])
			{
				if(g_config.getBool(ConfigManager::START_CHOOSEVOC))
				{
					...
					for(VocationsMap::iterator it = Vocations::getInstance()->getFirstVocation(); it != Vocations::getInstance()->getLastVocation(); ++it)
					{
						if(it->first == it->second->getFromVocation() && it->first != 0)
						{
							if(firstPart)
							{
								msg << "O que você quer ser... " << it->second->getDescription();
								firstPart = false;
							}
				...
				else if(!IOLoginData::getInstance()->playerExists(managerString, true))
				{
					...
					if(IOLoginData::getInstance()->createCharacter(managerNumber, managerString, managerNumber2, (uint16_t)managerSex))
						...
						msg << "Seu personagem não pôde ser criado, tente novamente.";
				}
				else
				{
					talkState[6] = true;
			...
			else if(talkState[11])
			{
				for(VocationsMap::iterator it = Vocations::getInstance()->getFirstVocation(); it != Vocations::getInstance()->getLastVocation(); ++it)
				{
					...
					{
						msg << "Então você gostaria de ser " << it->second->getDescription() << "... Você tem certeza?";
				...
				if(msg.str().length() == 17)
					msg << "Eu não entendo qual vocação que você gostaria de ser ... você poderia repetir isso?";
			}
			...
			else if(checkText(text, "no") && talkState[12])
			{
				talkState[11] = true;
				talkState[12] = false;
				msg << "Não? Então, o que você gostaria de ser?";
			}
			else if(checkText(text, "recovery key") && talkState[1])
			{
				talkState[1] = false;
				talkState[10] = true;
				msg << "Gostaria de uma chave de recuperação?";
			}
			else if(checkText(text, "yes") && talkState[10])
			{
				if(account.recoveryKey != "0")
					msg << "Desculpe, você já possui uma chave de recuperação, por razões de segurança, eu não posso dar uma nova.";
				else
				{
					managerString = generateRecoveryKey(4, 4);
					IOLoginData::getInstance()->setRecoveryKey(managerNumber, managerString);
					msg << "Sua chave de recuperação é: " << managerString << ".";
				}
			...
			else if(checkText(text, "no") && talkState[10])
			{
				...
				msg << "Por favor, leia a última mensagem que eu indiquei, não entendo a ação atual solicitada.";
		...
		case MANAGER_NEW:
		{
			...
			{
				msg << "O que você gostaria que seu login fosse?";
				talkState[1] = true;
				talkState[2] = true;
			}
			else if(talkState[2])
			{
				...
				if(tmp.length() < 6)
					...
					msg << "Sua senha contém caracteres inválidos ... por favor, me avise outro.";
				else
				{
					talkState[3] = true;
					talkState[2] = false;
					managerString = tmp;
					msg << managerString << " é isso? 'yes' ou 'no'?";
				}
			}
			else if(checkText(text, "yes") && talkState[3])
			{
				if(g_config.getBool(ConfigManager::GENERATE_ACCOUNT_NUMBER))
				{
					...
					if(id)
					{
						...
						msg << "Sua conta foi criada, você pode gerenciá-la agora, mas lembre-se do nome da sua conta: '"
							<< managerChar << "' E senha: '" << managerString
							<< "'! Se o nome da conta for muito difícil de lembrar, anote-o em algum lugar.";
					}
					else
						msg << "Sua conta não pôde ser criada, tente novamenten.";
				...
				{
					msg << "O que você gostaria que o nome da sua conta fosse?";
					talkState[3] = false;
					talkState[4] = true;
				}
			}
			else if(checkText(text, "no") && talkState[3])
			{
				talkState[2] = true;
				talkState[3] = false;
				msg << "O que você gostaria que sua senha fosse então?";
			}
			else if(talkState[4])
			{
				std::string tmp = text;
				trimString(tmp);
				if(tmp.length() < 3)
					msg << "Esse nome de conta é muito curto, pelo menos 3 dígitos são necessários. Selecione um nome de conta mais longo.";
				else if(tmp.length() > 25)
					msg << "Esse nome de conta é muito longo, não são necessários mais de 25 dígitos. Selecione um nome de conta mais curto.";
				else if(!isValidAccountName(tmp))
					msg << "O nome da sua conta contém caracteres inválidos, escolha outro.";
				else if(asLowerCaseString(tmp) == asLowerCaseString(managerString))
					msg << "O nome da sua conta não pode ser igual à senha, escolha outra.";
				else
				{
					sprintf(managerChar, "%s", tmp.c_str());
					msg << managerChar << ", Você tem certeza?";
					talkState[4] = false;
					talkState[5] = true;
				}
			}
			else if(checkText(text, "yes") && talkState[5])
			{
				if(!IOLoginData::getInstance()->accountNameExists(managerChar))
				{
					...
					if(id)
					{
						...
						msg << "Sua conta não pôde ser criada, tente novamente.";
				...
				{
					msg << "Uma conta com esse nome já existe, tente outro nome de conta.";
					talkState[4] = true;
					talkState[5] = false;
				}
			}
			else if(checkText(text, "no") && talkState[5])
			{
				talkState[5] = false;
				talkState[4] = true;
				msg << "O que mais você gostaria que o nome da sua conta?";
			}
			else if(checkText(text, "recover") && !talkState[6])
			{
				talkState[6] = true;
				talkState[7] = true;
				msg << "Qual foi o nome da sua conta?";
			}
			else if(talkState[7])
			{
				managerString = text;
				if(IOLoginData::getInstance()->getAccountId(managerString, (uint32_t&)managerNumber))
				{
					talkState[7] = false;
					talkState[8] = true;
					msg << "Qual foi sua chave de recuperação?";
				}
				else
				{
					msg << "Desculpe, mas a conta com esse nome não existe.";
			...
			else if(talkState[8])
			{
				...
				if(IOLoginData::getInstance()->validRecoveryKey(managerNumber, managerString2) && managerString2 != "0")
				{
					...
					msg << "Correto! Sua nova senha é: " << managerChar << ".";
				}
				else
					msg << "Desculpe, mas esta chave não corresponde à conta que você me deu.";
	...
	if(!noSwap)
		sendTextMessage(MSG_STATUS_CONSOLE_ORANGE, "Dica: Tipo 'account' Para gerenciar sua conta e se quiser começar de novo, digite 'cancel'.");
}
