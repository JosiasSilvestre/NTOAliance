...
bool ProtocolGame::parseFirstPacket(NetworkMessage& msg)
{
	...
	{
		ConnectionManager::getInstance()->addAttempt(getIP(), protocolId, false);
		disconnectClient(0x14, "Senha incorreta.");
		return false;
	}
...
void ProtocolGame::parseSay(NetworkMessage& msg)
{
	...
	if(text.length() > 700) //client limit
	{
		std::stringstream s;
		s << text.length();
...
{
	msg->put<char>(0xA0);
	if (player->getPlayerInfo(PLAYERINF﻿O_MAXHEALTH) > 0)
    {﻿
    	float f_h_percent = (float) player->getHealth()  / player->getPlayerIn﻿fo(PLAYERINFO_MAXHEALTH);
	...
	if(experience > 0x98967F) // client debugs after 9,999,999 exp
...
﻿
	...
	msg->put<char>(player->getPlayerInfo(PLAYERINFO_MAGICLEVELPER﻿CENT));
