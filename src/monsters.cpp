...
bool Monsters::deserializeSpell(xmlNodePtr node, spellBlock_t& sb, const std::string& description)
{
	...
	else
	{
		...
		else if(tmpName == "wind")
			combat->setParam(COMBATPARAM_COMBATTYPE, COMBAT_WINDDAMAGE);	
		...
		else if(tmpName == "skills" || tmpName == "attributes")
		{
			...
			if(readXMLInteger(node, "melee", intValue))
				...
				param = CONDITIONPARAM_SKILL_FISH;
				...
				param = CONDITIONPARAM_SKILL_FISHPERCENT;
			...
			while(tmpNode)
			{
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"immunity"))
				{
					if(readXMLString(tmpNode, "name", strValue))
					{
						...
						else if(tmpStrValue == "wind")
						{
							mType->damageImmunities |= COMBAT_WINDDAMAGE;
							mType->conditionImmunities |= CONDITION_WINDING;
						}

						else if(tmpStrValue == "undefineddamage")
						{
							mType->damageImmunities |= COMBAT_UNDEFINEDDAMAGE;
							mType->conditionImmunities |= CONDITION_PHYSICAL;
						}
					...
					else if(readXMLString(tmpNode, "earth", strValue) && booleanString(strValue))
					{
						mType->damageImmunities |= COMBAT_EARTHDAMAGE;
						mType->conditionImmunities |= CONDITION_EARTH;
					}
					else if(readXMLString(tmpNode, "poison", strValue) && booleanString(strValue))
					{
						mType->damageImmunities |= COMBAT_POISONDAMAGE;
						mType->conditionImmunities |= CONDITION_POISON;
					}
					...
					else if(readXMLString(tmpNode, "wind", strValue) && booleanString(strValue))
					{
						mType->damageImmunities |= COMBAT_WINDDAMAGE;
						mType->conditionImmunities |= CONDITION_WINDING;
					}
					
					else if(readXMLString(tmpNode, "undefineddamage", strValue) && booleanString(strValue))
					{
						mType->damageImmunities |= COMBAT_UNDEFINEDDAMAGE;
						mType->conditionImmunities |= CONDITION_PHYSICAL;
					}
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"elements"))
		{
			...
			while(tmpNode)
			{
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"element"))
				{
					...
					else if(readXMLInteger(tmpNode, "windPercent", intValue))
						mType->elementMap[COMBAT_WINDDAMAGE] = intValue;
