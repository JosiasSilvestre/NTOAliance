...
function onLogout(cid)
   ...
   if (team > 0) then
      ...
      if (getPlayerStorageValue(cid, CTF.Flagsto) > 0) then
         ...
         MsgToCTFM(22, getCreatureName(cid) .. " saiu do Capture a Bandeira sem entregar a bandeira do time ".. _CTF_LIB.flags[bant].na ..", por tanto ela foi devolvida!")
...
function onAttack(cid, target)     
   ...
   if (getPlayerStorageValue(cid, CTF.teamssto) > 0) then
      local team = getPlayerStorageValue(cid, CTF.teamssto)
      if (team == getPlayerStorageValue(target, CTF.teamssto)) then
         doPlayerSendCancel(cid, "Você nao pode atacar alguem que e do mesmo time que o seu!")
         return false
      end
   end      
   return true
