function onStepIn(cid, item, position, lastPosition, fromPosition, toPosition, actor)
	...
	elseif(#getZombiesEventPlayers() < getZombiesEventPlayersLimit() and getStorage(ZE_STATUS) == 1) then
		...
		if(players_on_arena_count == getZombiesEventPlayersLimit()) then
			...
			doBroadcastMessage(getCreatureName(cid) .. " entrou no evento assassino. Ainda faltam " .. getZombiesEventPlayersLimit() - players_on_arena_count .. " jogadores para dar inicio ao evento.")
		end
	else
