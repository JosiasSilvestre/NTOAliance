...
str = str .. ""..#players.." Jogadores"..(#players > 1 and "s" or "").." da equipe "..(getGlobalStorageValue(_Lib_Battle_Info.TeamOne.storage) == 0 and _Lib_Battle_Info.TeamTwo.name or _Lib_Battle_Info.TeamOne.name).." venceram o evento, e receberam um ".._Lib_Battle_Info.Reward[2].." "..getItemNameById(_Lib_Battle_Info.Reward[1]).."!"
for i = 1, #players do
...
doBroadcastMessage("[BattleField Event] Nos estamos esperando "..getGlobalStorageValue(_Lib_Battle_Info.storage_count).." jogadores para comecar a batalha.")
...
doBroadcastMessage("O evento não pode ser iniciado porque não havia jogadores suficientes.")
