...
function onSay(cid, words, param)
   ...
   if (param == "open") then
      if (getGlobalStorageValue(CTF.CTFSto) > 0) then
         return doPlayerSendTextMessage(cid, 27, "O Evento Roube a Bandeira ja esta aberto!")
      end
   ...
   elseif (param == "close") then
      if (getGlobalStorageValue(CTF.CTFSto) < 1) then
         return doPlayerSendTextMessage(cid, 27, "O Evento Roube a Bandeira ja esta fechado!")
      end
      ...
      for _, cid in pairs (getOnlinePlayers()) do       
          ...
          if (getPlayerStorageValue(cid, CTF.teamssto) > 0) then
             doPlayerSendTextMessage(cid, 22, "[Evento] Roube a Bandeira foi fechado e voce saiu do evento!")
      ...
      setGlobalStorageValue(CTF.CTFSto, -1)
      doPlayerSendTextMessage(cid, 27, "Evento Roube a Bandeira foi fechado com sucesso!")
   end 
   return true
end