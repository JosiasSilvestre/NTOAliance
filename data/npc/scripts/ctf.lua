...
function onCreatureSay(cid, type, msg)
   ...
   elseif (msgcontains(msg, "hi") and (focus == 0)) and getDistanceToCreature(cid) < 4 then
      ...
      if (_CTF_LIB.nopen) and (getGlobalStorageValue(_CTF_LIB.CTFSto) < 1) then
         selfSay("Ola {" .. getCreatureName(cid) .. "}, sou responsavel por levar os players para o evento ROUBAR A BANDEIRA, porem ele está fechado no momento.", cid)
      else
         selfSay("Ola {" .. getCreatureName(cid) .. "}, sou responsavel por levar os players para o evento ROUBAR A BANDEIRA, caso esteja interessado em ir diga {evento}.", cid)
   ...
   elseif (msgcontains(msg, "evento")) and (talkState[talkUser] == 0) and (focus == cid) and (getDistanceToCreature(cid) < 4) then
      ...
      if (_CTF_LIB.price) then
         selfSay("Para ir para o evento voce precisa pagar uma taxa de {".. _CTF_LIB.price .."} gold coins. Voce tem certeza que quer ir para o evento?", cid)
      else
         selfSay("Voce tem certeza que quer ir para o evento?", cid)
      end
      selfSay("Se voce tiver algum stealth ring ele será removido.", cid)
   elseif (msgcontains(msg, "sim") or msgcontains(msg, "yes")) and (talkState[talkUser] == 1) and (focus == cid) and (getDistanceToCreature(cid) < 4) then
      if (_CTF_LIB.nopen) and (getGlobalStorageValue(_CTF_LIB.CTFSto) < 1) then
         selfSay("Voce nao pode ir para o evento agora!", cid)
		 talkState[talkUser] = 0
      else
         talk_start = os.clock()
         if (_CTF_LIB.price) then
            if not(doPlayerRemoveMoney(cid, _CTF_LIB.price)) then
               selfSay("Voce precisa de {".. _CTF_LIB.price .. "} gold coins para poder ir para o evento :'(", cid)
               talk_start, talkState[talkUser] = os.clock(), 1
            else
                focus, talk_start, talkState[talkUser] = 0, 0, 0
