...
npcHandler:say("Me diga o nome do monstro que voce deseja fazer a task!", cid)
...
if not tonumber(contagem) then npcHandler:say('Desculpe, mas você ja terminou a task do '..msg, cid) return true end
setPlayerStorageValue(cid, tasktabble[msg].storage_start, 1)
npcHandler:say("Parabens, agora voce esta participando da missao task do "..msg..", falta matar "..string.sub(((contagem)-1)-tasktabble[msg].count, 2).." "..msg, cid)
...
npcHandler:say('digite o nome correto da missao task! para mais informacoes digite {!task}', cid)
talkState[talkUser] = 1
end
...
local contagem = getPlayerStorageValue(cid, v.storage)
if (contagem == -1) then contagem = 1 end
npcHandler:say('voce esta participando de uma missao task do '..k..' e ja tem '..((contagem)-1)..' '..k..' mortos, deseja mesmo sair?', cid)
end
end
else
npcHandler:say("você nao esta em nenhuma missao task", cid)
end
