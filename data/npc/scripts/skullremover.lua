...
selfSay('E ai o que vai ser? Remover {skulls} e o battle ou quer retirar só {battle}?', cid) 
talkState[talkUser] = 1 
elseif msgcontains(msg, 'skulls') and talkState[talkUser] == 1 then
if not S[getCreatureSkullType(cid)] then
selfSay('Você não esta com nenhuma skull!', cid)
talkState[talkUser] = 0 
else
selfSay('Voce quer remover sua skull por '..S[getCreatureSkullType(cid)]..' gps? {yes}', cid) 
...
selfSay('Voce nao tem '..S[getCreatureSkullType(cid)]..' gps suficientes!', cid)
talkState[talkUser] = 0 
end
elseif msgcontains(msg, 'battle') and talkState[talkUser] == 1 then
selfSay('Voce quer remover seu battle por '.. battle_price ..' gps? {yes}', cid) 
...
selfSay('Voce nao tem  '.. battle_price ..' gps suficientes!', cid)
talkState[talkUser] = 0
end
else
selfSay('Voce nao esta com battle!', cid)
talkState[talkUser] = 0
end
elseif msg == "no" and talkState[talkUser] >= 1 then 
selfSay("Entao nao", cid) 
talkState[talkUser] = 0 
