...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if(getItemLevelDoor(item.itemid) > 0) then
		if(item.actionid == 189) then
			if(not isPremium(cid)) then
				doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Apenas o digno pode passar.")
				return true
			end
	...
	if(isInArray(specialDoors, item.itemid)) then
		if(item.actionid == 100 or (item.actionid ~= 0 and getPlayerStorageValue(cid, item.actionid) > 0)) then
			...
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "A porta parece estar selada.")
		end
	...
	if(isInArray(keys, item.itemid)) then
		if(itemEx.actionid > 0) then
			...
			doPlayerSendCancel(cid, "A chave nao corresponde.")
			return true
		end
	...
	if(doors[item.itemid] ~= nil and checkStackpos(item, fromPosition)) then
		if(item.actionid == 0) then
			...
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Isso esta fechado.")
		end
