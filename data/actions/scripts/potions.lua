...
function onUse(cid, item, fromPosition, itemEx, toPosition) 
        ...
        then 
                doCreatureSay(itemEx.uid, "Somente " .. potion.vocStr .. (potion.level and (" de level " .. potion.level) or "") .. " ou acima pode beber este fluido.", TALKTYPE_ORANGE_1) 
                return TRUE 
        end 
        ...
        if(not realAnimation) then 
                ...
                for i, tid in ipairs(getSpectators(getCreaturePosition(cid), 1, 1)) do 
                        if(isPlayer(tid)) then 
                                doCreatureSay(itemEx.uid, "Me sinto melhor!", TALKTYPE_ORANGE_1, false, tid) 
                        end 
                end 
        end 
